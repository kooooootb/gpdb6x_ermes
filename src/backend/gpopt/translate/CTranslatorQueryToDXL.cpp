...
CTranslatorQueryToDXL::TranslateInsertQueryToDXL()
{
	...
	if (NULL != m_query->returningList)
	{
		...
		// this array is set in SELECT translation and is used to get colids by their index earlier
	...
	{
		// we can safely set it to empty array here as we don't outputting in this path
		m_dxl_query_output_cols = GPOS_NEW(m_mp) CDXLNodeArray(m_mp);
	}
